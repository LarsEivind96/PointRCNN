...
class KittiRCNNDataset(BumpDataset):
    def __init__(self, root_dir, npoints=16384, split='train', classes='reg_dump', mode='TRAIN', random_select=True,
        ...
        if classes == 'reg_dump':
            ...
            aug_scene_root_dir = os.path.join(root_dir, 'KITTI', 'aug_scene_reg')
            ...
            if gt_database_dir is not None:
                ...
                if cfg.GT_AUG_HARD_RATIO > 0:
                    ...
                    for k in range(self.gt_database.__len__()):
                        ...
                        # easy_list contains objects with more than 100 points
    ...
    # NB TODO: Do we need this? Maybe for adding different sizes of the bumps as classes
    def filtrate_objects(self, obj_list):
        ...
        for obj in obj_list:
            if obj.cls_type not in type_whitelist:  # rm Van, 20180928
                continue
            # Maybe this is not necessary, because the resolution doesn't change in our pcs
    ...
    def get_rpn_sample(self, index):
        ...
        # All points are valid... no need to check get_valid_flag
        ...
        # NB: Should we include DontCare objects? e.g. DontCare equals 
        if cfg.GT_AUG_ENABLED and self.mode == 'TRAIN':
            ...
            if np.random.rand() < cfg.GT_AUG_APPLY_PROB:
                # augment one scene
                # We don't have plane, this function relies on a plane
                gt_aug_flag, pts_rect, pts_intensity, extra_gt_boxes3d, extra_gt_obj_list = \
                    False, pts_rect, pts_intensity, None, None
                    # self.apply_gt_aug_to_one_scene(sample_id, pts_rect, pts_intensity, all_gt_boxes3d)

        # generate inputs
        # Reduce points from total number of points to npoints.
        if self.mode == 'TRAIN' or self.random_select:
            if self.npoints < len(pts_rect):
                pts_depth = pts_rect[:, 2]
                pts_near_flag = pts_depth < 999999999 # #NB: Changed this.. 40.0
            ...
            # TODO: Normalize the intensity in the bin files!
        ...
        # TODO: FIXED should be true or false?
        if cfg.RPN.FIXED:
            sample_info['pts_input'] = pts_input
    ...
    # Sets points within bbox to cls=1, points within buffer of bbox to cls=-1. Also returns the points (x,y,z,h,w,l,ry) within bbox
    @staticmethod
    ...
    # Used for data augmentation. Rotates box around y (vertical axis)
    ...
    # NB: F this. We don't include this as we don't have a plane. The function adds more bboxes for some reason.
    def apply_gt_aug_to_one_scene(self, sample_id, pts_rect, pts_intensity, all_gt_boxes3d):
        ...
        while try_times > 0:
            ...
            try_times -= 1
            # NB: Should we separate objects in easy and hard lists? Or don't we need it because the resolution in our dataset is the same overall?
            if cfg.GT_AUG_HARD_RATIO > 0:
                p = np.random.rand()
            ...
            if not valid_flag:
                continue
            # Else: the two bboxes does not intersect
